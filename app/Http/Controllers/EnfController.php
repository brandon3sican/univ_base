...
class EnfController extends Controller
{
    ...
    public function index()
    {
        ...
        $query = Enf::with(['ppa.recordType', 'indicator']);
        ...
        return view('sectors.enf.index', compact('enfs', 'offices', 'ppas', 'ppaName', 'recordId'));
    }
    ...
    public function store(Request $request): JsonResponse
    {
        try {
            ...
            $this->logEditHistory($enf, 'created', ['new' => $enfData]);

            return response()->json([
                'success' => true,
                'message' => 'ENF record created successfully',
                'data' => $enf->load(['ppa', 'indicator']),
            ]);
            ...
            Log::error('ENF Store Error: '.$e->getMessage());
    ...
    {
        $enf = Enf::with(['ppa', 'indicator'])->findOrFail($id);
    ...
    public function edit(string $id): JsonResponse
    {
        ...
        // Get all related ENF records for the same PPA
        $relatedEnfRecords = Enf::with(['ppa', 'indicator'])
            ->where('ppa_id', $enf->ppa_id)
            ->get();

        // Group data by office for easier frontend processing
        $officeData = [];
        foreach ($relatedEnfRecords as $record) {
        ...
        return response()->json([
            'enf' => $enf,
            'related_records' => $relatedEnfRecords,
    ...
    public function update(Request $request, string $id): JsonResponse
    {
        ...
        try {
            ...
            // Get current data
            $currentData = [
                'ppa_id' => $existingEnf->ppa_id,
                'indicator_id' => $existingEnf->indicator_id,
                // These are JSON columns with casts; keep as arrays.
                'office_id' => $existingEnf->office_id ?? [],
                'universe' => $existingEnf->universe ?? [],
                'accomplishment' => $existingEnf->accomplishment ?? [],
                'targets' => $existingEnf->targets ?? [],
                'years' => $existingEnf->years ?? [],
                'remarks' => $existingEnf->remarks,
            ];
            ...
            if ($indicatorText !== '') {
                if ($existingEnf->indicator_id) {
                    Indicator::whereKey($existingEnf->indicator_id)->update(['name' => $indicatorText]);
                    $currentData['indicator_id'] = $existingEnf->indicator_id;
                } else {
            ...
            error_log("ENF UPDATE - ID: $id, Section: $editSection");
            error_log('ENF UPDATE - Data: '.json_encode($updateData));

            try {
                $result = $existingEnf->update($updateData);
                if (! $result) {
                    throw new \Exception('Update returned false (no changes persisted)');
                }
                error_log('ENF UPDATE - Result: SUCCESS');
                error_log('ENF UPDATE - Updated record: '.json_encode($existingEnf->fresh()->toArray()));

                // Log edit history
                $changes = [
                    'old' => array_intersect_key($existingEnf->getOriginal(), $updateData),
                    'new' => $updateData,
                ];
                $this->logEditHistory($existingEnf, 'updated', $changes);
            } catch (\Exception $updateException) {
                error_log('ENF UPDATE - Update Exception: '.$updateException->getMessage());
            ...
            return response()->json([
                ...
                'data' => $existingEnf->load(['ppa', 'indicator']),
            ]);
        } catch (\Exception $e) {
            return response()->json([
                'success' => false,
    ...
    public function destroy(string $id): JsonResponse
    {
        ...
        $this->logEditHistory($enf, 'deleted', ['deleted' => $enf->toArray()]);

        $enf->delete();
        
        // Check if PPA is used by other records
        $ppaUsageCount = Enf::where('ppa_id', $ppaId)->count();
    ...
    public function compare(Request $request)
    {
        ...
        foreach ($enfData as $enf) {
            $accomplishments = $enf->accomplishment ?? [];
            $years = $enf->years ?? [];
            $officeIds = $enf->office_id ?? [];
            $universe = $enf->universe ?? [];
        ...
        return view('sectors.enf.compare', compact(
            'year1',
            'year2',
            'year1Total',
            'year2Total',
