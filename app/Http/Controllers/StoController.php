...
class StoController extends Controller
{
    public function index()
    {
        ...
        $stoRecords = Sto::whereNull('parent_id')
        ...
        return view('sto.index', compact('stoRecords'));
    }
    ...
    public function store(Request $request)
    {
        try {
            ...
            $validated = $request->validate([
                ...
                'parent_id' => 'nullable|integer|exists:sto,id',
            ...
            // Set order_column
            $maxOrder = Sto::max('order_column') ?? 0;
    ...
    {
        try {
            $record = Sto::findOrFail($id);

            // Get the record with the next higher order (lower order_column value)
            $previousRecord = Sto::where('order_column', '<', $record->order_column)
    ...
    public function moveDown($id)
    {
        try {
            ...
            $nextRecord = Sto::where('order_column', '>', $record->order_column)
    ...
    public function reorder(Request $request)
    {
        try {
            ...
            $draggedRecord = Sto::findOrFail($draggedId);
            $targetRecord = Sto::findOrFail($targetId);

            // Get all records ordered by current order
            $allRecords = Sto::orderBy('order_column')->get();
