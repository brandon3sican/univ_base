        ...
        <div class="bg-gradient-to-r from-slate-700 to-slate-800 text-white p-3 shadow-lg">
            <div class="flex items-center justify-between">
                <div class="flex items-center">
                    ...
                    <h2 class="text-sm font-bold">SOILCON Data Table</h2>
                </div>
                <div class="flex items-center space-x-2">
                    ...
                    <a href="{{ route('soilcon.compare') }}"
                        class="bg-teal-600 hover:bg-teal-700 text-white px-3 py-1.5 rounded-lg transition-all duration-200 text-xs font-medium shadow hover:shadow-lg hover:border-2 hover:border-white flex items-center">
                    ...
                    <tr>
                        <td colspan="100%" class="p-3 bg-gray-50 border-t border-gray-300">
                            <div class="grid grid-cols-1 md:grid-cols-3 gap-3">
                                @php
                                    $totalRecords = $soilcons->count();
                                    $recordsWithIndicator = $soilcons->whereNotNull('indicator_id')->count();
                                    $currentYear = date('Y');
                                    $recordsWithNoCurrentYearAccomplishment = 0;

                                    foreach ($soilcons as $soilcon) {
                                        // Only check records that have an indicator
                                        if ($soilcon->indicator_id === null) {
                                            continue;
                                        }

                                        $hasCurrentYearAccomplishment = false;
                                        $accomplishment = $soilcon->accomplishment ?? [];
...
<script>
    ...
    // Load SOILCON data for the table
    function loadSoilconData() {
        fetch('/soilcon', {
                headers: {
            ...
            .catch(error => console.error('Error loading SOILCON data:', error));
    }

    // Load form data (PPAs and Indicators)
    function loadFormData() {
        return fetch('/soilcon/create', {
                headers: {
                    'Accept': 'application/json',
                    'Content-Type': 'application/json'
                }
            ...
            .catch(error => {
                console.error('Error loading SOILCON data:', error);
    ...
    function populateSelects() {
        ...
        // Add PPA options (only SOILCON PPAs - types_id = 6)
        if (ppaSelect) {
            ppas.forEach(ppa => {
                // Only show PPAs that belong to SOILCON (types_id = 6)
                if (ppa.types_id === 6) {
    ...
    function filterPpaByRecordType() {
        ...
        fetch(`/api/soilcon/ppas?record_type_id=${selectedRecordTypeId}`)
            .then(response => response.json())
            .then(data => {
                ...
                // Add filtered PPAs to dropdown (only SOILCON PPAs - types_id = 6)
    ...
    function formatUniverseData(universe, targets) {
        ...
        if (typeof universe === 'object' && universe !== null) {
            ...
            if (values.length > 0) {
                ...
                // Create CAR tooltip (sum of all office targets per year)
                let carTooltip = `<div class="font-bold text-lg text-blue-600 mb-4">CAR</div>`;
                carTooltip += `<div class="font-semibold text-white mb-3">Total Targets per Year:</div>`;

                if (targets && typeof targets === 'object') {
                    targetYears.forEach(year => {
                        const yearTotal = officeIds.reduce((sum, officeId) => {
                            const officeTargets = targets[officeId];
                            return sum + (officeTargets && officeTargets[year] !== null &&
                                officeTargets[year] !== undefined ? officeTargets[year] : 0);
                        }, 0);
                        const isFuture = year > currentYear;
                        const isCurrent = year === currentYear;
                        let yearClass = 'font-medium';
                        ...
                        carTooltip +=
                            `<div class="text-sm py-2">• ${yearLabel}: <span class="${yearClass}">${formatNumberWithCommas(yearTotal)}</span></div>`;
                    });
                } else {
                    carTooltip += `<div class="text-sm py-2">No targets set</div>`;
                }
    ...
    document.getElementById('crudForm').addEventListener('submit', function(e) {
        ...
        if (data.ppa_id === 'new') {
            ...
            submitFormData.append('types_id', '6'); // SOILCON type
        ...
        const url = currentEditId ? `/soilcon/${currentEditId}` : '/soilcon';
    ...
    function openCreateModal() {
        currentEditId = null;
        document.getElementById('modalTitle').textContent = 'Add New SOILCON Record';
