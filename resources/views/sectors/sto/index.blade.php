        ...
        <div class="bg-gradient-to-r from-slate-700 to-slate-800 text-white p-3 shadow-lg">
            <div class="flex items-center justify-between">
                <div class="flex items-center">
                    ...
                    <h2 class="text-sm font-bold">STO Data Table</h2>
                </div>
                <div class="flex items-center space-x-2">
                    ...
                    <a href="{{ route('sto.compare') }}"
                        class="bg-purple-600 hover:bg-purple-700 text-white px-3 py-1.5 rounded-lg transition-all duration-200 text-xs font-medium shadow hover:shadow-lg hover:border-2 hover:border-white flex items-center">
                    ...
                    <tr>
                        <td colspan="100%" class="p-3 bg-gray-50 border-t border-gray-300">
                            <div class="grid grid-cols-1 md:grid-cols-3 gap-3">
                                @php
                                    $totalRecords = $stos->count();
                                    $recordsWithIndicator = $stos->whereNotNull('indicator_id')->count();
                                    $currentYear = date('Y');
                                    $recordsWithNoCurrentYearAccomplishment = 0;

                                    foreach ($stos as $sto) {
                                        // Only check records that have an indicator
                                        if ($sto->indicator_id === null) {
                                            continue;
                                        }

                                        $hasCurrentYearAccomplishment = false;
                                        $accomplishment = $sto->accomplishment ?? [];
                                        $officeIds = $sto->office_id ?? [];
...
<script>
    ...
    document.getElementById('crudForm').addEventListener('submit', function(e) {
        ...
        if (data.ppa_id === 'new') {
            ...
            submitFormData.append('types_id', '2'); // Default type - controller can adjust if needed
            ...
            .then(data => {
                ...
                loadFormData().then(() => {
                    ...
                    setTimeout(() => {
                        ...
                        // Only use current record's office data for selection
                        ...
                        // Only select offices from the current record
                        currentRecordOfficeIds.forEach(officeId => {
