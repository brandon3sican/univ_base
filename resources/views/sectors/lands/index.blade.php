        ...
        <div class="bg-gradient-to-r from-slate-700 to-slate-800 text-white p-3 shadow-lg">
            <div class="flex items-center justify-between">
                <div class="flex items-center">
                    ...
                    <h2 class="text-sm font-bold">LANDS Data Table</h2>
                </div>
                <div class="flex items-center space-x-2">
                    ...
                    <a href="{{ route('lands.compare') }}"
                        class="bg-amber-600 hover:bg-amber-700 text-white px-3 py-1.5 rounded-lg transition-all duration-200 text-xs font-medium shadow hover:shadow-lg hover:border-2 hover:border-white flex items-center">
                    ...
                    <tr>
                        <td colspan="100%" class="p-3 bg-gray-50 border-t border-gray-300">
                            <div class="grid grid-cols-1 md:grid-cols-3 gap-3">
                                @php
                                    $totalRecords = $lands->count();
                                    $recordsWithIndicator = $lands->whereNotNull('indicator_id')->count();
                                    $currentYear = date('Y');
                                    $recordsWithNoCurrentYearAccomplishment = 0;

                                    foreach ($lands as $land) {
                                        // Only check records that have an indicator
                                        if ($land->indicator_id === null) {
                                            continue;
                                        }

                                        $hasCurrentYearAccomplishment = false;
                                        $accomplishment = $land->accomplishment ?? [];
                                        $officeIds = $land->office_id ?? [];
