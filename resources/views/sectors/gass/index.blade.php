        ...
        <div class="bg-gradient-to-r from-slate-700 to-slate-800 text-white p-3 shadow-lg">
            <div class="flex items-center justify-between">
                <div class="flex items-center">
                    ...
                    <h2 class="text-sm font-bold">GASS Data Table</h2>
                </div>
                    ...
                    <tr>
                        <td colspan="100%" class="p-3 bg-gray-50 border-t border-gray-300">
                            <div class="grid grid-cols-1 md:grid-cols-3 gap-3">
                                @php
                                    $totalRecords = $gasses->count();
                                    $recordsWithIndicator = $gasses->whereNotNull('indicator_id')->count();
                                    $currentYear = date('Y');
                                    $recordsWithNoCurrentYearAccomplishment = 0;

                                    foreach ($gasses as $gass) {
                                        // Only check records that have an indicator
                                        if ($gass->indicator_id === null) {
                                            continue;
                                        }

                                        $hasCurrentYearAccomplishment = false;
                                        $accomplishment = $gass->accomplishment ?? [];
                                        $officeIds = $gass->office_id ?? [];
...
<script>
    ...
    // Load GASS data for the table
    function loadGassData() {
        fetch('/gass', {
                headers: {
            ...
            .catch(error => console.error('Error loading GASS data:', error));
    }

    // Load form data (PPAs and Indicators)
    function loadFormData() {
        return fetch('/gass/create', {
                headers: {
                    'Accept': 'application/json',
                    'Content-Type': 'application/json'
                }
    ...
    function populateSelects() {
        ...
        // Add PPA options (only GASS PPAs - types_id = 1)
        if (ppaSelect) {
            ppas.forEach(ppa => {
                // Only show PPAs that belong to GASS (types_id = 1)
                if (ppa.types_id === 1) {
        ...
        if (indicatorTextarea) {
            ...
            // Initially hide office section
            const officeSection = document.getElementById('officeSection');
            if (officeSection) {
                officeSection.style.display = 'none';
            }
            // Don't call updateButtonVisibility() here - it will be called when needed
        }
    }
    ...
    function filterPpaByRecordType() {
        ...
        fetch(`/api/ppas?record_type_id=${selectedRecordTypeId}`)
            .then(response => response.json())
            .then(data => {
                ...
                // Add filtered PPAs to dropdown (only GASS PPAs - types_id = 1)
    ...
    function getIndentedPPAName(item) {
        ...
        switch (recordType.toUpperCase()) {
            ...
            case 'MAIN ACTIVITY':
                ...
                if (!mainActivityCounter[programCounter][projectCounter[programCounter]]) {
                    mainActivityCounter[programCounter][projectCounter[programCounter]] = 0;
                }
                ...
                if (!majorActivityCounter[programCounter][projectCounter[programCounter]]) {
                    majorActivityCounter[programCounter][projectCounter[programCounter]] = {};
                }
                ...
                if (!subActivityCounter[programCounter][projectCounter[programCounter]]) {
                    subActivityCounter[programCounter][projectCounter[programCounter]] = {};
                }
            ...
            case 'MAJOR ACTIVITY':
                ...
                if (!majorActivityCounter[programCounter][projectCounter[programCounter]][currentMain]) {
                ...
                if (!subActivityCounter[programCounter][projectCounter[programCounter]][currentMain]) {
            ...
            case 'SUB ACTIVITY':
                ...
                if (!subActivityCounter[programCounter][projectCounter[programCounter]][currentMainForSub]) {
            ...
            case 'ACTIONABLE TASK':
                ...
                if (!subActivityCounter[programCounter] || !subActivityCounter[programCounter][projectCounter[
                        ...
                        currentMainForTask
                    ] || !subActivityCounter[programCounter][projectCounter[programCounter]][currentMainForTask][
                        currentMajorForTask
                    ]) {
                    numbering = '';
                    className = 'font-normal text-sm ml-30';
    ...
    document.getElementById('crudForm').addEventListener('submit', function(e) {
        ...
        const url = currentEditId ? `/gass/${currentEditId}` : '/gass';
    ...
    function populateOfficeSpecificData(officeData) {
        ...
        // Also populate data from the current record itself
        const record = window.editData?.gass;
        console.log('Current record:', record);

        Object.keys(officeData).forEach(officeId => {
            const data = officeData[officeId];
            console.log(`Processing office ${officeId}:`, data);
            ...
            // Populate accomplishment for each year
            ...
            // Populate targets for each year
        ...
        // Also populate data directly from the record if available
        if (record) {
            console.log('Populating from record:', record);

            // Populate universe from record
            if (record.universe) {
                let universeValue = record.universe;
                if (typeof universeValue === 'string') {
                    try {
                        universeValue = JSON.parse(universeValue);
                    } catch (e) {
                ...
                if (universeValue && typeof universeValue === 'object') {
                    Object.keys(universeValue).forEach(officeId => {
                        const universeInput = document.querySelector(`input[name="universe[${officeId}]"]`);
                        if (universeInput) {
                            universeInput.value = universeValue[officeId] ?? '';
                        }
            ...
            // Populate accomplishment from record
            if (record.accomplishment) {
                let accomplishmentValue = record.accomplishment;
                if (typeof accomplishmentValue === 'string') {
                    try {
                        accomplishmentValue = JSON.parse(accomplishmentValue);
                    } catch (e) {
                ...
                console.log('Record accomplishment:', accomplishmentValue);
                if (accomplishmentValue && typeof accomplishmentValue === 'object') {
                    // Structure is {officeId: {year: value}}
                    Object.keys(accomplishmentValue).forEach(officeId => {
                        const officeData = accomplishmentValue[officeId];
                        if (officeData && typeof officeData === 'object') {
                            Object.keys(officeData).forEach(year => {
                                const yearValue = officeData[year];
                                ...
                                if (accompInput) {
                                    accompInput.value = yearValue ?? '';
                                    console.log(
                                        `Set record accomplishment[${officeId}][${year}] = ${yearValue}`
                                    );
                                }
                            });
                        }
            ...
            // Populate targets from record
            if (record.targets) {
                let targetsValue = record.targets;
                if (typeof targetsValue === 'string') {
                    try {
                        targetsValue = JSON.parse(targetsValue);
                    } catch (e) {
                ...
                console.log('Record targets:', targetsValue);
                if (targetsValue && typeof targetsValue === 'object') {
                    // Structure is {officeId: {year: value}}
                    Object.keys(targetsValue).forEach(officeId => {
                        const officeData = targetsValue[officeId];
                        if (officeData && typeof officeData === 'object') {
                            Object.keys(officeData).forEach(year => {
                                const yearValue = officeData[year];
                                ...
                                if (targetInput) {
                                    targetInput.value = yearValue ?? '';
                                    console.log(
                                        `Set record targets[${officeId}][${year}] = ${yearValue}`
                                    );
                                }
                            });
                        }
            ...
            // Populate remarks from record
            const remarksTextarea = document.getElementById('remarks');
            if (remarksTextarea && record.remarks) {
    ...
    function openCreateModal() {
        currentEditId = null;
        document.getElementById('modalTitle').textContent = 'Add New GASS Record';
        ...
        // Reset button states - updateButtonVisibility will handle the correct state
        const saveBtn = document.getElementById('saveBtn');
        const nextBtn = document.getElementById('nextBtn');
        if (saveBtn) {
            saveBtn.classList.remove('hidden');
        ...
        // Call updateButtonVisibility after loading data to set correct button state
        updateButtonVisibility();
    }
