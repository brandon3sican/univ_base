        ...
        <div class="bg-gradient-to-r from-slate-700 to-slate-800 text-white p-3 shadow-lg">
            <div class="flex items-center justify-between">
                <div class="flex items-center">
                    ...
                    <h2 class="text-sm font-bold">Biodiversity Data Table</h2>
                </div>
                <div class="flex items-center space-x-2">
                    ...
                    <a href="{{ route('biodiversity.compare') }}"
                        class="bg-emerald-600 hover:bg-emerald-700 text-white px-3 py-1.5 rounded-lg transition-all duration-200 text-xs font-medium shadow hover:shadow-lg hover:border-2 hover:border-white flex items-center">
                    ...
                    <tr>
                        <td colspan="100%" class="p-3 bg-gray-50 border-t border-gray-300">
                            <div class="grid grid-cols-1 md:grid-cols-3 gap-3">
                                @php
                                    $totalRecords = $biodiversities->count();
                                    $recordsWithIndicator = $biodiversities->whereNotNull('indicator_id')->count();
                                    $currentYear = date('Y');
                                    $recordsWithNoCurrentYearAccomplishment = 0;

                                    foreach ($biodiversities as $biodiversity) {
                                        // Only check records that have an indicator
                                        if ($biodiversity->indicator_id === null) {
                                            continue;
                                        }

                                        $hasCurrentYearAccomplishment = false;
                                        $accomplishment = $biodiversity->accomplishment ?? [];
                                        $officeIds = $biodiversity->office_id ?? [];
...
<script>
    ...
    document.getElementById('crudForm').addEventListener('submit', function(e) {
        ...
        if (data.ppa_id === 'new') {
            ...
            submitFormData.append('types_id', '4'); // Default type - controller can adjust if needed
    ...
    function populateOfficeSpecificData(officeData) {
        ...
        const record = window.editData?.biodiversity;
