<script>
    function toggleActionsColumn() {
        ...
        // Sync with cloned header if it exists
        const clonedActionsHeader = document.getElementById('cloned-thead')?.querySelector('#actionsHeader');
        if (clonedActionsHeader) {
            if (actionsHeader.classList.contains('hidden')) {
                clonedActionsHeader.classList.add('hidden');
            } else {
