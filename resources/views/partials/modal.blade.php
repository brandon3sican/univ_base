    ...
    <div class="relative min-h-full flex items-center justify-center p-4">
        <div class="bg-white rounded-lg shadow-xl w-full max-w-6xl max-h-[90vh] overflow-y-auto">
            ...
            <form id="crudForm" class="px-6 py-4 space-y-4">
                ...
                <input type="hidden" id="type" name="type" value="{{ $sectorType }}">
                <input type="hidden" id="editMode" name="editMode" value="false">

                <!-- Edit Mode Section Selection (shown only in edit mode) -->
                <div id="editSectionSelector" class="hidden">
                    <label class="block text-sm font-medium text-gray-700 mb-3">What would you like to edit?</label>
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                        <div class="flex items-center p-3 border border-gray-200 rounded-lg hover:bg-gray-50 cursor-pointer"
                            onclick="selectEditSection('ppa')">
                            <input type="radio" name="editSection" value="ppa" id="editSectionPpa" class="mr-3">
                            <label for="editSectionPpa" class="cursor-pointer flex-1">
                                <div class="font-medium text-gray-900">PPA Details</div>
                                <div class="text-xs text-gray-500">Change PPA, record type, or create new PPA</div>
                            </label>
                        </div>
                        <div class="flex items-center p-3 border border-gray-200 rounded-lg hover:bg-gray-50 cursor-pointer"
                            onclick="selectEditSection('data')">
                            <input type="radio" name="editSection" value="data" id="editSectionData" class="mr-3">
                            <label for="editSectionData" class="cursor-pointer flex-1">
                                <div class="font-medium text-gray-900">Office Assignment & Data</div>
                                <div class="text-xs text-gray-500">Offices, universe, accomplishments, targets, remarks
                                </div>
                            </label>
                        </div>
                    </div>
                ...
                <!-- PPA Details Section -->
                <div id="ppaDetailsSection">
                    <!-- Record Type Selection -->
                    <div>
                        <label for="record_type_id" class="block text-sm font-medium text-gray-700 mb-1">Record
                            Type</label>
                    ...
                    <div>
                        ...
                        <select id="ppa_id" name="ppa_id"
                            ...
                            @foreach ($ppas as $ppa)
                                @if ($ppa->types_id == $sectorType)
                                    <option value="{{ $ppa->id }}" title="{{ $ppa->name }}">
                                        {{ \Illuminate\Support\Str::limit($ppa->name, 100, '...') }}</option>
                                @endif
                            @endforeach
                ...
                <!-- Office Assignment & Data Section -->
                <div id="dataSection" class="hidden">
                    <!-- Office Selection -->
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-2">Office Assignment</label>
                        <div class="grid grid-cols-2 md:grid-cols-3 gap-2">
                            ...
                            <div class="flex items-center">
                                ...
                                <label for="selectAllOffices"
                                    class="text-sm text-gray-700 cursor-pointer font-semibold">Select/Unselect
                                    All</label>
                            </div>
                            ...
                            <div>
                                ...
                                <div class="overflow-x-auto border border-gray-200 rounded-lg shadow-sm">
                                    <table class="min-w-full">
                                        <thead
                                            class="bg-gradient-to-r from-blue-50 to-indigo-50 border-b border-gray-200">
                                            <tr>
                                                <th
                ...
                <div class="flex justify-end space-x-3 mt-6">
                    ...
                    <button type="submit" id="saveBtn" {{ $saveButtonDisabled ? 'disabled' : '' }}
                        class="px-4 py-2 {{ $saveButtonDisabled ? 'bg-gray-400 text-white opacity-50 cursor-not-allowed' : 'bg-blue-600 text-white hover:bg-blue-700' }} rounded-lg transition-colors {{ $saveButtonDisabled ? 'hidden' : '' }}">
...
<div id="deleteModal" class="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full hidden z-50">
    <div class="relative top-20 mx-auto p-5 border w-96 shadow-lg rounded-lg bg-white">
        <div class="mt-3 text-center">
            ...
            <p class="text-sm text-gray-500 mb-4">Are you sure you want to delete this {{ $sectorName }} record?
                This action
                cannot
                be undone.</p>
